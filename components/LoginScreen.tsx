...
import { Ruler, Mail, CheckCircle2 } from "lucide-react";
...
export default function LoginScreen() {
  ...
  async function handleSendLink(e: React.FormEvent) {
    ...
    try {
      ...
      setError(e?.message || "Could not send the magic link. Please try again.");
    } finally {
      setSending(false);
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-paper px-4">
      <div className="w-full max-w-sm bg-surface border border-border rounded-md shadow-card p-8">
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <div className="text-sm text-ink-soft mt-1">Sign in with your work email</div>
        </div>
        ...
        {sent ? (
          <div className="text-center py-4">
            ...
            <div className="text-sm text-ink-soft mt-1">
              We sent a sign-in link to <span className="font-medium text-ink">{email}</span>. Click it to open the
              app.
            </div>
          </div>
        ) : (
