...
import { formatQty, formatCurrency } from "@/lib/calculations";
                  ...
                  <ul className="space-y-2">
                    {rows.map((h) => (
                      <li key={h.id} className="border border-border rounded p-3">
                        ...
                        {type === "Invoice" && h.invoice_amount != null && (
                          <div className="flex items-center justify-between mt-1 text-xs text-ink-faint">
                            <span>Invoice Amount</span>
                            <span className="font-mono text-rust">{formatCurrency(h.invoice_amount)}</span>
                          </div>
                        )}
                      </li>
                    ))}
