...
import { Printer, FileDown, Pencil, Trash2 } from "lucide-react";
import type { DocumentWithLines } from "@/hooks/useDocuments";
import { Button, TagChip } from "@/components/ui";
import { formatDate } from "@/lib/utils";
import { formatCurrency, formatQty } from "@/lib/calculations";
...
export default function DocumentDetail({
  ...
  /** Admin-only Edit/Delete actions — omitted (default false) leaves this component exactly as before. */
  canManage?: boolean;
  onEdit?: () => void;
  onDelete?: () => void;
}) {
    ...
    <div className="bg-surface border border-border rounded-md shadow-card">
      <div className="flex items-start justify-between px-5 py-4 border-b border-border print:hidden">
        ...
        <div className="flex gap-2">
          ...
          {canManage && onDelete && (
            <Button variant="secondary" onClick={onDelete} className="hover:text-danger">
      ...
      <div id="document-print-area" className="px-5 py-4">
        ...
        <div className={`grid ${doc.entry_type === "Invoice" ? "grid-cols-4" : "grid-cols-3"} gap-4 mb-5 text-sm`}>
          <div>
          ...
          {doc.entry_type === "Invoice" && (
            <div>
              <div className="text-xs text-ink-faint mb-0.5">Invoice Amount</div>
              <div className="text-ink font-mono">{doc.invoice_amount != null ? formatCurrency(doc.invoice_amount) : "—"}</div>
            </div>
          )}
        </div>
