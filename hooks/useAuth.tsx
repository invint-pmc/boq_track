...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const resolveAuthorization = useCallback(async (email: string | undefined | null) => {
    ...
    // Three conditions must all hold: (1) the email exists in
    // authorized_users, (2) the row is active, (3) a role is assigned.
    // Any failure -> denied, regardless of a valid Supabase Auth session.
